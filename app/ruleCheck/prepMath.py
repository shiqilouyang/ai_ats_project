...
class PreMath(Maths):
    ...
    def primarySchool(self):
        ...
        r = s = ""
        try:
            if self.studyMoudelmixper is None:
                self.ik_s = '没有学习模块'
            else:
                ...
                for i in self.studyMoudelintervictiveper:
                    for k, v in i.items():
                        if v > int(studyModeNum.split("_")[-1]):
                            s += k + ","
            ...
            if self.restudyMoudelmixper is None:
                ...
                for i in self.restudyMoudelmvper:
                    for k, v in i.items():
                        if v > int(self.restudyModeNum.split("_")[1]):
                            s += k + ","
                    self.ik_r += "重学习" + r + " 知识点 视频个数 超过规则个数" if s else ""
                r = ""
        except Exception as e:
            self.ik_r = '重学习模块出现异常'
        if 1:
                        ...
                        for k, v in i.items():
                            ...
                            if 4 <= len(v) < 5 :
                                if i.get(k)[-1].ability < float(self.season_subject__first.study_ability) :
                                    ...
                                    s = 1
                    self.ik_s += "做了4道题,但是最后一道能的能力值小于{} ".format(
        ...
        self.html_data.update({
            ...
            "restudymodeRule": self.season_subject__first.restudymodeRule,
            "rate": self.rate,
            "ik_t": "无测试模块" if self.ik_t == "" else self.ik_t,
                        ...
                        for k, v in i.items():
                            ...
                            if len(v) <= 4 :
                                if i.get(k)[-1].ability < float(self.season_subject__first.study_ability)  :
                                    ...
                                    s = 1
                    self.ik_r += "做了4道题,但是最后一道能的能力值小于{} ".format(
                        float(self.season_subject__first.study_ability)) if s else ""
                    s = 0
            except Exception as e:
                self.ik_r = "重学习模块出现异常"
...
#     user_id = "autoTest-xfl-4684-1609828748"
#     session_id = "336586367098657792"
#     group_id = "20502"
