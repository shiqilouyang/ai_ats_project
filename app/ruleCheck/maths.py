...
class Maths(RuleChecks):
    ''' 数学章节系列 '''

    def __init__(self, user_id, session_id, group_id):
        super(Maths, self).__init__(user_id, session_id, group_id)
        self.mathsMiddleware()

    def mathsMiddleware(self):
        print(" In Maths this is userId {}".format(self.user_id))
        ...
        if self.studyMoudelmixper:
            for i in self.studyMoudelmixper:
                for k,v in i.items():
                    for l in v:
                        if l.usage == 'LO' or l.usage == 'BASIC':
                            v.remove(l)
                    for l in v:
                        if  l.usage == 'INTERACT':
                            v.remove(l)
